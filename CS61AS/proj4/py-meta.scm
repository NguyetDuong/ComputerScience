;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; CS61AS Project 4
;; Person A: Eui Jin "Jeanie" Jung
;; Person B: Nguyet Minh Duong (CS61AS-EU)
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	(cond ((infix-op? token) ;; arithmetic infix operators
        ...
        ((or? token)
            (if (ask val 'true?)
                ...
                (py-eval line-obj env)))
        ;; test for membership
        ;; TODO: Person B, Question 5
        ;; Need to get all items AFTER the command "contains" aka the LIST
        ;; ask LIST contains the value
        ((in? token)
         (let ((check (eval-item line-obj env))) ;; this takes everything afterwards command
                (ask check '__contains__ val)))  ;; use the scheme verson to get true/false & return
	      ;; TODO: Person B, QUestion 5
        ;; Need to get AFTER the command "contains" aka LIST
        ;; Note that the next item needs to be "in"
        ;; ask LIST contains value
        ;; if true, returns false, else true
        ((not? token)
	       (let ((wd (ask line-obj 'next)) ;; not follows by "in", need to remove this first
                (check (eval-item line-obj env)))
                (negate-bool (ask check '__contains__ val)))) 
...
;; TODO: Both Partners, Question 8
;; Returns a list of cons'd obj pairs
;; want it to look like ((a . b) (c . d))
;; Should have helper methods?
;; MethodX will return a LIST where everytime it sees a comma, it will return '()
;; It will "lookahead" and checks if it is a close-token
;; if it is, we just return the evaluated statement & do not keep looking
;; It will lookahead to check if it is a comma, if it is a comma, we
;; evaluate the first val, then the second and cons it together
(define (collect-key-value line-obj env close-token)
  (let ((check (ask line-obj 'next)))
    (cond ((comma? check) 
      (collect-key-value line-obj env close-token)) ;; if it's a comma, keep reading -- you're "done"
      ((eq? check close-token) '()) ;; if it is == close-token, stop reading
      (else (ask line-obj 'push check)
            (let ((key (py-eval line-obj env)))
              (if (colon? (ask line-obj 'peek))
                (begin (ask line-obj 'next) ;; remove the colon
                        (let ((val (py-eval line-obj env)))
                          (cons (cons key val) (collect-key-value line-obj env close-token))
                        ))
                (py-error "SyntaxError: Expected comma to separate key-value pairs")))))))


;; (cons (cons 1 2) (cons (cons 3 4) (cons (cons 5 6) '()))
;; -> ((1 . 2) (3 . 4) (5 . 6))
;; what we want
...
;; Conditionals
;; Needs to return ('list '*BLOCK* '*IF-BLOCK* predicate (if (elif (elif ... ))))
;; TODO: Person B, Question 7

(define (get-if-pred line-obj env)
  (cons (ask line-obj 'indentation) (make-statement line-obj env)))

(define (make-if-block line-obj env)
  (let ((predicate (get-if-pred line-obj env))
        (body (split-block (read-block (ask line-obj 'indentation) env))))
        (list '*BLOCK* '*IF-BLOCK* predicate body)))
;; should be like this...
;; cdr (*IF-BLOCK* predicate (if (elif (elif ... ))))
;; cddr ( predicate (if (elif (elif ... ))))
;; caddr predicate 
;; cddr (if )
...
(define (if-block-body block)
  (let ((body-and-else (cadddr block)))
    (car body-and-else)))
...
(define (eval-if-block block env)
  (let ((pred (make-line-obj (if-block-pred block)))
        (body (if-block-body block)))
    (let ((boolean-value (ask (py-eval pred env) 'true?))
      ...
      (define (loop)
        (cond (boolean-value
              ...
              ((eq? else-clause #f)
                *NONE*)
            (else (eval-item (make-line-obj else-clause) env))))
    (loop))))
...
;; since else if, is essentially if
;; we want it to function and turn it into multiple if-statement


(define (make-elif-block line-obj env)
  (let ((predicate (get-if-pred line-obj env))
       (body (split-block (read-block (ask line-obj 'indentation) env))))
       (list '*BLOCK* '*ELIF-BLOCK* predicate body)))
...
(define (eval-elif-block block env)
  (let ((str-elif (helper block)))
        (eval-if-block str-elif env)))

;; We want it to evaluate as if it's an if-statement
;; so just go change the tag since that determines where it goes
(define (helper block)
  (cond ((null? block)
          '())
        ((eq? '*ELIF-BLOCK* (car block))
          (cons '*IF-BLOCK* (helper (cdr block))))
        (else 
          (cons (car block) (helper (cdr block))))))
...
;; While loops
;; Both Partners, Question 6
(define (make-while-block line-obj env)
  (let ((predicate (get-while-pred line-obj env))
        (body (split-block (read-block (ask line-obj 'indentation) env))))
      (list '*BLOCK* '*WHILE-BLOCK* predicate body)))
;; in relative it looks like (*BLOCK* *WHILE-BLOCK* predicate (body else/#f))
;; cdr -> '*WHILE-BLOCK* predicates body
;; cddr -> predicates body
;; caddr -> predicates  
;; cadddr -> (body else/#f)
...
;; need a way to get predicates
;; while x = 3:
;; so we skip the "while", get everything afterwards -- aka next
;; put it into a list, up until colon
(define (get-while-pred line-obj env)
  (cons (ask line-obj 'indentation) (make-statement line-obj env)))

(define (make-statement line-obj env)
  (let ((wd (ask line-obj 'next)))
      (if (colon? wd)
          '()
          (cons wd (make-statement line-obj env)))))
...
;; For loops
;; TODO: Person A, Question 7
(define (make-for-block line-obj env)
  (let ((var (ask line-obj 'next))
        (collection (begin (ask line-obj 'next) (get-for-collection line-obj env)))  
        (body (split-block (read-block (ask line-obj 'indentation) env))))
      (list '*BLOCK* '*FOR-BLOCK* var collection body)))

;helper procedure
(define (get-for-collection line-obj env)
  (let ((token (ask line-obj 'next)))
    (if (colon? token)
        '()
        (cons token (get-for-collection line-obj env)))))
...
(define (for-block-body block)
  (let ((body-and-else (cddddr block)))
    (caar body-and-else)))
...
(define (eval-for-block block env)
    (let ((for-var (for-block-var block))
          (for-collection (for-block-collection block))
          (for-body (for-block-body block))
          (for-else (for-block-else block)))
    (let ((result-of-iter (ask (py-eval (instantiate line-obj '*DUMMY-INDENT* for-collection) env) '__iter__ for-var for-body env)))
      (cond ((eq? result-of-iter '*BREAK*) (begin ((set! for-else #f) *NONE*)))
            ((eq? for-else #t) (eval-sequence for-else env))
            (else *NONE*)))))
