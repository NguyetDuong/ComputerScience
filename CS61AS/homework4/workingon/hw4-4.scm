...
(define (sub-interval x y)
  (make-interval (abs (- (car x) (cdr y))) (abs (- (cdr x) (car y))))
...
(define (percent inval)
	(define c (center inval))
	(define hms (- (upper-bound inval) c))
	(* (/ hms c) 100)
)
(define (make-center-percent c p)
	(define dif (* c (/ p 100)))
	(if (> (+ c dif) (- c dif))
		(make-interval (- c dif) (+ c dif))
		(make-interval (+ c dif) (- c dif)))
)
...
(define (for-each proc l)
	(if (null? l)
		nil
		(cons (proc (car l)) (for-each proc (cdr l)))
	))
...
(define (substitute sentenceList oldWd newWd)
  (cond ((null? sentenceList) '())
	...
	((equal? oldWd (car sentenceList))
	 (cons newWd (substitute (cdr sentenceList) oldWd newWd)))
	(else (cons (car sentenceList) (substitute (cdr sentenceList) oldWd newWd))))

)


; Exercise 3 - Define substitute2
(define (substitute2 sentList oldWdList newWdList)
	(if (null? oldWdList)
		sentList
		(substitute2 (substitute sentList (car oldWdList) (car newWdList)) (cdr oldWdList) (cdr newWdList)))
