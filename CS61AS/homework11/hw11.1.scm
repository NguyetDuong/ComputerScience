...
(define (dispatch-eval exp env)
  (define (type-tag l)
    (car l))
  (cond ((self-evaluating? exp) exp)
	((variable? exp) (lookup-variable-value exp evn))
	((eq? (type-tag exp) 'quote) (text-of-quotation exp))
	((eq? (type-tag exp) 'set!) (eval-assignment exp env))
	((eq? (type-tag exp) 'define) (eval-definition exp env))
