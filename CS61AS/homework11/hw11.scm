...
#|
I am essentially looking at only the first environment and then if 
there is no environment, it throws an error. Else, I will recreate
items and make it into something else 
#|

(define (make-unbound! items env)
  (if (null? (first-frame env))
      (error "There is nothing in here to change")
    (set! items '()))
)
...
#|
Using the implemented version, he can configure it to work the way he wants to. 
However in Louis' situation, the system version of map cannot decode everything
being input into the eval and as a result will cause a problem.
|#
; SICP 4.15
; Explain why it is impossible for the halts? procedure to exist.
#|
This problem is a giant contradiction because technically try does not 
continue and will stop. However because of this, halts doesn't correctly do its job
and will allow try to continue "going in a loop" by returning false
but try cannot do this, and as a result, it will stop.
|#
