...
map:
	...
	lw $a0, 0($s0) ### YOUR CODE HERE ###
	# call the function on that value.
	jalr $s1 ### YOUR CODE HERE ###
	# store the returned value back into the node
	# need to get the "lo" from this. How?? 81
	sw $v0, 0($s0) ### YOUR CODE HERE ###
	# load the address of the next node into $a0
	lw $a0, 4($s0)     ##addu $a0, $s0, $4 -- does not work because you're adding 4 to the address ### YOUR CODE HERE ###
	# put the address of the function back into $a1 to prepare for the recursion
	addu $a1, $s1, $0 ### YOUR CODE HERE ###
	#recurse
	jal map ### YOUR CODE HERE ###
