...
unsigned write_pass_one(FILE* output, const char* name, char** args, int num_args) {
    if (strcmp(name, "li") == 0) {
      ...
      long int immediate;
      int err = translate_num(&immediate, args[1], -2147483648, 4294967296);
      if (err == -1) {
        return 0;
      }
      if (-1 != translate_num(&immediate, args[1], -32768, 32768)) {
...
int write_lui(uint8_t opcode, FILE* output, char** args, size_t num_args) {
  ...
  uint32_t instruction = (uint32_t) opcode;
...
int write_addiu(uint8_t opcode, FILE* output, char** args, size_t num_args) {
    ...
    int err = translate_num(&immediate, args[2], -32768, 32768);
...
int write_branch(uint8_t opcode, FILE* output, char** args, size_t num_args, 
  ...
  if (num_args != 3) {
    ...
    uint32_t instruction = (uint32_t) opcode;
    instruction = instruction<<26 | rs<<21 | rt<<16 | (offset & 0x0000ffff);
    write_inst_hex(output, instruction);
    return 0;
}
...
int write_rtype(uint8_t funct, FILE* output, char** args, size_t num_args) {
    ...
    uint32_t instruction = (uint32_t) funct;
