...
# Returns: none
#------------------------------------------------------------------------------
hex_to_str:	# Begin hex_to_str
	# YOUR CODE HERE
	addiu $t2 $0 28	# Set shift amount
	addiu $t0 $0 -1 # set up the mask
	srl $t0 $t0 28	# ...
	sll $t0 $t0 28	# ...
	j hex_to_str_skip_decrement	# Don't want to decrement until actually start looping
hex_to_str_loop:
	beq $t2 $0 hex_to_str_end	# If shift already zero, have seen whole int
	addiu $t2 $t2 -4 	# Decrement shift amount
	srl $t0 $t0 4 	# Shift mask
hex_to_str_skip_decrement:
	and $t1 $a0 $t0	# Set $t1 to the first 4 bits of the int using mask
	srlv $t1 $t1 $t2 	# shift "int" so that know what first hexal is
	addiu $t7 $t1 -9
	blez $t7 hex_to_str_nine_or_less	# Determine if hexal a digit or a letter
	andi $t1 $t1 96	# Hexals a-f symbol
	sb $t1 0($a1)
	addiu $a1 $a1 1
	j hex_to_str_loop # Hexal added, loop for next hexal
hex_to_str_nine_or_less:
	andi $t1 $t1 48	# Hexals 0-9 symbol
	sb $t1 0($a1) 
	addiu $a1 $a1 1
	j hex_to_str_loop	# Hexal added, loop for next hexal
hex_to_str_end:
	addiu $t7 $0 10 # newline
	sb $t7 0($a1)
	addiu $a1 $a1 1
	addu $t7 $0 $0 	# NULL
	sb $t7 0($a1)
