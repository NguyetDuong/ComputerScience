...
inst_needs_relocation:
	# YOUR CODE HERE
	addu $v0 $0 $0	# set the return value to deafault 0
	srl $a0 $a0 26	# shift to leave only the opcode
	addiu $t0 $0 2 	# opcodes of j and jal
	addiu $t1 $0 3 	# ...
	beq $a0 $t0 inst_needs_relocation_yes
	beq $a0 $t1 inst_needs_relocation_yes
	j inst_needs_relocation_no
inst_needs_relocation_yes:
	addiu $v0 $0 1 	# set return value to 1
inst_needs_relocation_no:
	jr $ra
...
# 	(Returns an int that is the new instruction.)
#------------------------------------------------------------------------------
relocate_inst:
	# YOUR CODE HERE
	addiu $sp, $sp, -12
	...
	addu $s0 $a0 $0	# Save the instruction
	addu $s1 $a2 $0 # save the symbol table
	addu $a0 $a3 $0 # setup for symbol_for_addr
	jal symbol_for_addr # retrieve name of wanted symbol
	beq $v0 $0 relocate_inst_error
	addu $a0 $s1 $0	# setup for addr_for_symbol
	addu $a1 $v0 $0
	jal addr_for_symbol	# retrieve absolute adress of symbol
	addiu $t0 $0 -1
	beq $v0 $t0 relocate_inst_error
	sll $v0 $v0 4	# shift absolute adress so that it fits
	srl $v0 $v0 6
	srl $s0 $s0 26	# shift the instruction to eliminate unwanted bits
	sll $s0 $s0 26
	and $v0 $v0 $s0	# the new instruction
	j relocate_inst_end
