...
(define-class (py-list val)
  ...
  (method (__setitem__ index item)
    ...
    (let ((n (ask index 'val)))
      ...
      (if (< n 0) (set! n (+ n (length val))))
      (replace-item n val)))
  ;; TODO: Person A, Question 4  
  (method (__contains__ other)
     (let ((inside (ask self 'val)))
        (if (null? (filter (lambda (x) (ask x 'true?)) (map (lambda (obj) (py-apply (ask obj '__eq__) (list other))) inside)))
            *PY-FALSE*
            *PY-TRUE*)))
  (method (append)
