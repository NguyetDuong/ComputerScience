...
class MutableBoard extends Board {
    ...
    /** The amount of moves made on this specific board. */
    ...
    /**This is to be used in the beginning step, where the whole board is filled
     * with WHITE-sides and only one dot in the middle.
    ...
    /**
     * A board whose initial contents are copied from BOARD0, but whose undo
     * history is clear.
     */
    ...
    /**
     * This will take in a Board and take everything inside that board and
     * transfer it onto a Square[][] (the basis for an actual MutableBoard).
    ...
    void addSpot(Side player, int r, int c) {
        ...
        if (!isOverfull(r, c)) {
            ...
            _board[r - 1][c - 1] = Square.square(player, newSpots);
        } else {
            Square copy = get(r, c);
            int newSpots = copy.getSpots() + 1;
            if (numOfSide(player) == _size * _size) {
                ...
                _board[r - 1][c - 1] = Square.square(player, copy.getSpots()
                        - neighbors(r, c) + 1);
    ...
    /**
     * This is the addSpot(...) helper in order for us to be better at keeping
     * track of the Undo option! This does exactly the same thing as
     * addSpot(...) with the exception that this is a continuation of
     * addSpot(...) where it is still on the same turn while addSpot(...)
    ...
    /**
     * This is to called when we are to add one into all the Squares around the
     * Square in the position (r, c).
     * @param r
     * @param c
     * @param s
     */
    ...
    /**
     * Returns true if the number of spots is greater than the amount of
     * neighbors around the Square containing these spots. The reason for this
     * is, there must be greater than the neighbors to put one in every
     * neighboring square.
     * @param spots
     * @param r
     * @param c
    ...
    /**
     * This is a helper method for addSpots(...) where it will return true if,
     * when added another spot onto the current solution, the spot will become
     * "overfull" -- bigger #of spots than all the Squares around it.
     * @param r
     * @param c
     * @return boolean
     */
    boolean isOverfull(int r, int c) {
        ...
        if (r == 1) {
            if (c == 1) {
                ...
                return (spotsBigger(actual, toRight) && spotsBigger(actual,
                        toBottom));
            } else if (c == _size) {
                Square toLeft = get(r, c - 1);
                Square toBottom = get(r + 1, c);
                return (spotsBigger(actual, toLeft) && spotsBigger(actual,
                        toBottom));
            } else {
                Square toLeft = get(r, c - 1);
                Square toBottom = get(r + 1, c);
                ...
                return (spotsBigger(actual, toLeft)
                        && spotsBigger(actual, toBottom) && spotsBigger(actual,
                            toRight));
            }
        } else if (r == _size) {
            if (c == 1) {
                ...
                Square toLeft = get(r, c - 1);
                return (spotsBigger(actual, toLeft)
                        && spotsBigger(actual, toTop) && spotsBigger(actual,
                            toRight));
            }
        } else if (c == 1) {
            Square toTop = get(r - 1, c);
            ...
            Square toLeft = get(r, c - 1);
            Square toBottom = get(r + 1, c);
            return (spotsBigger(actual, toLeft) && spotsBigger(actual, toTop)
                    && spotsBigger(actual, toRight)
    ...
    /**
     * This is a helper method for isOverFull(...) where it takes in two
     * Squares, and then compare the two Squares -- returning true if the amount
     * of spots in Square a is bigger than the amount of spots in Square b.
     * @param a
     * @param b
    ...
    /**
     * Set the contents of the square with index IND to SQ. Update counts of
