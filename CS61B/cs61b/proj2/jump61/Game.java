...
class Game extends Observable {
    ...
    int play() {
        ...
        _out.flush();
        _exit = -10;
        Player person = new HumanPlayer(this, Side.RED);
        Player computer = new AI(this, Side.BLUE);
        _players[0] = person;
        _players[1] = computer;
        while (_exit < 0) {
            checkForWin();
            Side turn = _board.whoseMove();
            if (_players[0].type().equals("AI")
                    && _players[1].type().equals("AI")
                    && _playing) {
                if (turn.equals(Side.RED)) {
                    _players[0].makeMoveHuman();
                } else if (turn.equals(Side.BLUE)) {
                    _players[1].makeMove();
                }
            } else {
                if (turn.equals(Side.RED)
                    && _players[0].type().equals("AI") && _playing) {
                    _players[0].makeMoveHuman();
                } else if (turn.equals(Side.BLUE)
                    && _players[1].type().equals("AI") && _playing) {
                    _players[1].makeMove();
                } else {
                    if (promptForNext()) {
                        readExecuteCommand();
    ...
    void makeMove(int r, int c) {
        ...
        if (_board.isLegal(_board.whoseMove(), r, c)) {
            ...
            _out.println("Error: invalid move: " + r + " " + c);
        }
    }
    ...
    void makeMove(int n) {
        ...
        if (_board.isLegal(_board.whoseMove(), n)) {
            ...
            _out.println("Error: invalid move: " + n);
        }
    }
    ...
    private void checkForWin() {
        int totalSq = _board.size() * _board.size();
        if (_board.numOfSide(Side.BLUE) == totalSq
                || _board.numOfSide(Side.RED) == totalSq) {
            announceWinner();
            _playing = false;
        }
    }
    ...
    private void setAuto(Side color) {
        try {
            Player comp = new AI(this, color);
            if (color.equals(Side.RED)) {
                _players[0] = comp;
                _playing = false;
            } else if (color.equals(Side.BLUE)) {
                _players[1] = comp;
                _playing = false;
            }
        } catch (Exception e) {
            _out.println("Error: syntax error in 'auto' command");
        }
    }
    ...
    private void setManual(Side color) {
        try {
            Player realPerson = new HumanPlayer(this, color);
            if (color.equals(Side.BLUE)) {
                _players[1] = realPerson;
                _playing = false;
            } else if (color.equals(realPerson)) {
                _players[0] = realPerson;
                _playing = false;
            }
        } catch (Exception e) {
            _out.println("Error: syntax error in 'auto' command");
    ...
    private void setPlayer(Side color, Player player) {
        Player A = getPlayer(color);
        Player B = new HumanPlayer(this, A.getSide());
        player = B;
    }
    ...
    private void setSpots(int r, int c, int spots, String color) {
        ...
        if (color.equals("b")) {
            ...
            _out.println("Error: syntax error in 'set' command");
        }
        if (spots < 0 || spots > _board.neighbors(r, c)) {
            _out.println("Error: invalid request to put "
    ...
    private void setSize(int n) {
        if (n < 2 || n > 10) {
            _out.println("Error: size must be between 2 and 10");
        } else {
            _board.clear(n);
        }
        announce();
    ...
    private void readExecuteCommand() {
        if (!_playing) {
            String cmd = _inp.next();
            try {
                executeCommand(cmd);
            } catch (Exception e) {
                _out.println("Error: syntax error in '<" + cmd + ">' command");
            }
            ...
            try {
                ...
                try {
                    ...
                    _out.println("Error: move "
                        + r + " " + c + " out of bounds");
                }
                if (_board.whoseMove().equals(Side.BLUE)) {
                    _players[1].makeMove();
                } else {
                    _players[0].makeMove();
                }
            } catch (Exception e) {
                _out.println("Error: syntax error in '<move>' command");
            }
        }
        _inp.nextLine();
    }
    ...
    private void executeCommand(String cmnd) {
        ...
        case "start":
            ...
            break;
        default:
            _out.println("Error: bad command " + cmnd);
        }
    }
