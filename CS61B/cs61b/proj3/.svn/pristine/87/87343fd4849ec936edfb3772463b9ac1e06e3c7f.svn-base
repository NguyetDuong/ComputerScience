...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        ...
        for (int i = 0; i < segment.size(); i++) {
            if (i == segment.size() - 1) {
                Road currentRoad = _map.getLabel(segment.get(i - 1),
                        segment.get(i));
                Location end = _map.getLabel(segment.get(i));
                System.out.println(directionNum + ". Take "
                        + currentRoad.toString() + " "
                        + currentRoad.direction().fullName() + " for "
                        + Math.round(savedDistance * 10.0) / 10.0
                        + " miles to " + end.toString() + ".");
                directionNum++;
            } else {
                Road currentRoad = _map.getLabel(segment.get(i),
                        segment.get(i + 1));
                if (savedRoad != null
                        && savedRoad.toString().equals(currentRoad.toString())
                        && savedRoad.direction()
                                .equals(currentRoad.direction())) {
    ...
    /** This is the number of the direction. */
