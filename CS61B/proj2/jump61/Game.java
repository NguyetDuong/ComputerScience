...
class Game extends Observable {
    ...
    int play() {
        ...
        while (_exit < 0) {
    	 ...
    	 // The bottom three were originally not in the else statement
    	 _prompter.close();
         _out.close();
         _err.close();
        // Ending point
    ...
    void makeMove(int n) {
        ...
        if (_board.isLegal(_board.whoseMove(), n)) {
        	...
        	_err.println("Error: invalid move: " + n);
        }
        // FIXME
    }
    ...
    private void readExecuteCommand() {
        // FIXME
    	promptForNext();
    	while (_inp.hasNext("") || _inp.hasNext("\n") || _inp.hasNext(" ")) {
    		_inp.next();
    	}
    	if (!_inp.hasNextInt() && _playing) {
    		String check = _inp.next();
    		while (!Arrays.asList(COMMAND_NAMES).contains(check)) {
    			check = _inp.next();
    		}
    ...
    private void placeCards(int a, int b) {
    	
    	System.out.println("The amount of times lol#3");
    	try {
